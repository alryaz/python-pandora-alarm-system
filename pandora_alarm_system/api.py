...
__all__ = ("PandoraOnlineAccount", "PandoraOnlineDevice")
...
class PandoraOnlineAccount:
    ...
    def _update_device_current_state(
        ...
        # filter new state arguments
        state = device.state
        new_state_args = CurrentState.filter_timestamp_args(
            state, **state_args
        )
        if differences := (state_args.keys() - new_state_args.keys()):
            self.logger.warning(
                f"Ignoring updates on attributes due to "
                f"timestamp misalignment: {', '.join(sorted(differences))}"
            )
        state_args = new_state_args

        # Create new state if not present
        if state is None:
            state = CurrentState(**state_args)
            self.logger.debug(
                f"Setting new state object on device {device.device_id}"
            )
        else:
            self.logger.debug(
                f"Oldest update: {state.oldest_update_timestamp}"
            )
            self.logger.debug(
                f"Newest update: {state.newest_update_timestamp}"
            )

            state_args = new_state_args
            state = attr.evolve(state, **state_args)
            self.logger.debug(
        ...
        if data_stats:
            ...
            update_args.update(
                **CurrentState.get_common_dict_args(
                    data_stats,
                    state_timestamp=data_stats.get("dtime"),
